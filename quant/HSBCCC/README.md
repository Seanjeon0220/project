# HSBC Private Bank Case Challenge 2026 — Portfolio Toolkit

A calculation toolkit built for the HSBC Private Bank Case Challenge 2026. It takes a private-banking client's situation — an $80M portfolio with $45M in cash plus external assets — and works it through to a full investment proposal: strategic and tactical asset allocation, fund selection, a credit solution, and checks against the client's income and education goals.

## What it does

The notebook runs in two phases.

**Phase A — inputs and metrics.** Sets up the fund universe (44 traditional funds from Appendix I, 9 alternatives from Appendix II), computes Sharpe and advisory-fee-adjusted metrics for each, and builds a 12×12 asset-class correlation matrix from WRDS/CRSP monthly returns, extended to cover private markets and hedge funds.
...
Correlation matrix from WRDS/CRSP monthly returns (Feb 2021 – Dec 2024). Fund data from the case materials (Appendix I traditional funds, Appendix II alternatives). WRDS access is required to re-run the correlation cells.